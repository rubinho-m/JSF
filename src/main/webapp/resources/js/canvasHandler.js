...
function mouseDownHandler(event) {
    ...
    if (flagR) { // R выбран
        ...
        setPoint(requestX, requestY, chosenR)
...
//
    ...
    try {
        for (let i = 1; i < rows.length; i++) {
            ...
            drawX = cellX / actualR * divX + width / 2
            drawY = -(cellY / actualR * divY) + height / 2


            let result = checkChangedR(cellX, cellY, actualR)
...
function drawChart(context, width, height, R, color) {
    ...
    context.beginPath()
    context.moveTo(width / 2, height / 2)
    context.arc(width / 2, height / 2, R, Math.PI, 3 * Math.PI / 2)
    context.fill()
...
const triangle = [[width / 2, height / 2], [width / 2, (height / 2 - R)], [width / 2 + R, height / 2]]
const rect = [[width / 2 - R * 2, height / 2], [width / 2, height / 2], [width / 2, height / 2 + 2 * R], [width / 2 - 2 * R, height / 2 + 2 * R]]
...
function setPoint(x, y, R) {
    console.log(x, y)
    document.getElementById('j_idt41:xHiddenValue').value = x
    document.getElementById('j_idt41:yHiddenValue').value = y
    document.getElementById('j_idt41:RHiddenValue').value = R
    document.getElementById('j_idt41:submitHidden').click()


}

document.getElementById('j_idt10:number').addEventListener('beforeinput', function () {
    console.log(document.getElementById('j_idt10:output').innerText)
})

function checkGraphInfo(R) {
    document.getElementById('graphicInfo').innerText = 'График нарисован для значения R: ' + R
}

function checkChangedR(x, y, R) {
    return isInCircle(x, y, R) || isInTriangle(x, y, R) || isInRect(x, y, R)
}
